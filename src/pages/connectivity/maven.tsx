...
function ArtifactsTable({artifacts}: { artifacts: MavenArtifact[] }) {
  return <Paper>
    <Toolbar sx={{ pl: 3, pr: 3 }}>
      <Typography sx={{ flex: "1 1 100%" }} component="div" variant="h5">
        Repositories
...
function ArtifactRow({ artifact }: { artifact: MavenArtifact }) {
  const [editingMode, setEditingMode] = useState(false);
  const [modifiedData, setModifiedData] = useState<MavenArtifact | null>(
    null,
  );
  const [sendUpdate, mutationResult] = useUpdateMavenArtifactMutation();
  const [deleteArtifact, deletionResult] = useDeleteMavenArtifactMutation();

  function handleDelete() {
    if (artifact) {
      deleteArtifact({ groupId: artifact.groupId, artifactId: artifact.artifactId });
    }
  }
  function save() {
    setEditingMode(false);
    if (modifiedData) {
      sendUpdate({
        groupId: modifiedData.groupId,
        artifactId: modifiedData.artifactId,
        mavenArtifact: modifiedData,
      }).then(() => setModifiedData(null));
    }
  }
